...
namespace Nox7atra.Utils
{
    public static class MeshTools
    {
        ...
        public static MeshFilter CreateMeshObj(string name, Material mat)
        {
            ...
            var bounds = meshObj.GetComponent<MeshFilter>();
            var sizeX  = bounds.transform.localScale.x;
            var sizeY  = bounds.transform.localScale.y;
            float sizeZ = bounds.transform.localScale.z;
            meshObj.AddComponent<BoxCollider>().size = new Vector3(sizeX, sizeY, sizeZ);
            mr.material = mat;
            meshObj.name = name;
            return mf;
        }
